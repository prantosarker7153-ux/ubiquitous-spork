...
Select an option below:"""
    ...
    # ============================================================
        ...
        per_page = 5
        all_videos = get_all_direct_videos(limit=500)
        total = len(all_videos)
        ...
        if not page_videos:
            await query.edit_message_text(
                "📋 **File IDs List**\n\n"
                "📭 কোনো video upload করা নেই।\n\n"
                "Bot এ video পাঠালে এখানে File ID দেখাবে।",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔙 Back", callback_data="admin_main")
                ]]),
                parse_mode=ParseMode.MARKDOWN
        ...
        text = f"📋 **File IDs List** (মোট {total}টি) — পাতা {page+1}\n\n"
        text += "👇 নিচের File ID গুলো tap করে copy করুন:\n\n"
        keyboard = []
        
        for v in page_videos:
            code = v['code']
            title = v.get('title', 'Untitled')[:22]
            media_type = v.get('media_type', 'video')
            views = v.get('views', 0)
            icon = "🎬" if media_type == 'video' else "📄" if media_type == 'document' else "🎞️"
            
            text += f"{icon} **{title}**\n"
            text += f"   ID: `{code}` | 👁 {views} views\n\n"
            
            # Copy button shows the file ID in a popup (copyable)
            keyboard.append([
                InlineKeyboardButton(
                    f"📋 Copy: {code}",
                    callback_data=f"fileid_copy_{code}"
                )
            ])
        ...
        nav_buttons = []
        if page > 0:
            nav_buttons.append(InlineKeyboardButton("⬅️ আগের পাতা", callback_data=f"admin_file_ids_page_{page-1}"))
        if end_idx < total:
            nav_buttons.append(InlineKeyboardButton("পরের পাতা ➡️", callback_data=f"admin_file_ids_page_{page+1}"))
        if nav_buttons:
            keyboard.append(nav_buttons)
    ...
    elif data.startswith("fileid_copy_"):
        if user_id != ADMIN_ID:
            return
        code = data.replace("fileid_copy_", "")
        bot_info = await context.bot.get_me()
    ...
    # ============================================================
        ...
        text = f"""📤 **Forward Channels** (মোট {count}টি)

Bot এ যা upload হবে সব এই channel গুলোতে automatically forward হবে।
...
"""
        ...
        if fwd_channels:
            for ch in fwd_channels:
                ...
                text += f"✅ **{ch_name}**\n   ID: `{ch_id}`\n\n"
                keyboard.append([
                    InlineKeyboardButton(f"🗑️ Remove: {ch_name[:20]}", callback_data=f"fwd_remove_{ch_id}")
                ])
        else:
        ...
        query.data = "admin_forward_channel"
        await button_callback(update, context)
...
👇 <b>Tap the number below to copy:</b>"""
        ...
        # 🔥 Auto-forward to ALL forward channels
        fwd_channels = get_forward_channels()
        if fwd_channels:
